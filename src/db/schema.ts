...
export const users = pgTable("users", {
  ...
  phoneInput: varchar("phone_input", { length: 40 }),
  phoneCountry: varchar("phone_country", { length: 2 }),
  phoneNeedsReview: boolean("phone_needs_review").default(false).notNull(),
...
// Better Auth's core adapter contract includes these models, even with its
// generic credential and verification endpoints disabled.
export const authAccounts = pgTable("auth_accounts", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("auth_accounts_user_idx").on(table.userId)]).enableRLS();
...
export const authVerifications = pgTable("auth_verifications", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("auth_verifications_identifier_idx").on(table.identifier)]).enableRLS();
